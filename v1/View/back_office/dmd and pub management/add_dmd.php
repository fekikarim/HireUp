...
if (
    ...
    if (
        ...
        header('Location: dmd_management.php?success_global=' . urlencode($success_message) . '&titre=' . urlencode($dmd->get_titre()));
        exit(); // Make sure to stop further execution after redirection
    } else {
        // returning an error
        $error_message = "Failed to add the reqeust. Please try again later.";
        header('Location: dmd_management.php?error_global=' . urlencode($error_message));
        exit(); // Make sure to stop further execution after redirection
    }
} else {
